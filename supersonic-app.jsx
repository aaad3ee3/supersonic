...
function RashqSection({
  ...
  return /* @__PURE__ */<div><div className="flex items-center justify-between px-4 mt-7 mb-3"><div className="flex items-center gap-2"><Zap className="w-4 h-4 text-purple-400" /><h3 className="text-white font-bold text-[15px]">رشق — متابعين وتفاعل</h3></div>{apiState === "error" && /* @__PURE__ */<span className="text-[10px] text-amber-500">أسعار تقديرية — الباك اند غير متصل</span>}{apiState === "loading" && /* @__PURE__ */<span className="text-[10px] text-gray-500">يحدّث الأسعار...</span>}</div><div className="px-4 space-y-5">{RASHQ_PLATFORMS.map(p => /* @__PURE__ */<div key={p.key}><div className="flex items-center gap-1.5 mb-2"><p.Icon className="w-3.5 h-3.5 text-purple-400" /><p className="text-purple-300 text-xs font-bold">{p.label}</p></div><div className="space-y-2">{p.services.map(s => {
            ...
            return /* @__PURE__ */<button key={s.id} onClick={() => onOpenService(s, p.label, unitPrice, isLive)} className="w-full flex items-center justify-between bg-surface border border-purple-900 rounded-xl px-3.5 py-3"><ChevronLeft className="w-4 h-4 text-gray-600 shrink-0" /><span className="flex-1 text-right px-3"><span className="block text-gray-200 text-xs">{s.name}</span><span className="block text-purple-500 text-[10px] mt-1">من {fmt(unitPrice)}$ / 1000</span></span></button>;
          })}</div></div>)}</div></div>;
}
...
function ProductSheet({
  ...
  return /* @__PURE__ */<div className="fixed inset-0 z-30 flex items-end justify-center"><div className="absolute inset-0 bg-void" style={{
      ...
      overflowY: "auto"
            ...
            return /* @__PURE__ */<button key={i} onClick={() => setSelected(p)} className={active ? "rounded-2xl p-3.5 text-right border bg-purple-600 border-purple-400 flex items-center gap-3" : "rounded-2xl p-3.5 text-right border bg-void border-purple-900 flex items-center gap-3"}>{p.image && /* @__PURE__ */<img src={p.image} alt="" loading="lazy" className="w-10 h-10 rounded-lg object-cover shrink-0 bg-gray-900" />}<span className="flex-1 min-w-0"><span className="block text-white text-sm font-bold mb-1 truncate">{p.label}</span><span className={active ? "block text-purple-100 text-xs" : "block text-purple-500 text-xs"}>${fmt(unitPriceOf(p))}</span></span></button>;
      ...
      maxWidth: 1100
        ...
        minHeight: "100vh"
          ...
          margin: "0 auto"
              ...
              setMode("login");
              setForgotSent(false);
            }} className="text-purple-400 text-xs font-bold">رجوع لتسجيل الدخول</button></div> : /* @__PURE__ */<form onSubmit={submit} className="space-y-3">{mode === "signup" && /* @__PURE__ */<div className="flex items-center gap-2 bg-surface border border-purple-900 rounded-xl px-4 py-3"><User className="w-4 h-4 text-purple-500" /><input value={name} onChange={e => setName(e.target.value)} placeholder="\u0627\u0633\u0645\u0643" className="bg-transparent outline-none text-sm text-white placeholder-gray-500 flex-1" /></div>}<div className="flex items-center gap-2 bg-surface border border-purple-900 rounded-xl px-4 py-3"><Mail className="w-4 h-4 text-purple-500" /><input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="\u0627\u0644\u0628\u0631\u064A\u062F \u0627\u0644\u0625\u0644\u0643\u062A\u0631\u0648\u0646\u064A" className="bg-transparent outline-none text-sm text-white placeholder-gray-500 flex-1" dir="ltr" /></div>{mode !== "forgot" && /* @__PURE__ */<div className="flex items-center gap-2 bg-surface border border-purple-900 rounded-xl px-4 py-3"><Lock className="w-4 h-4 text-purple-500" /><input type={showPass ? "text" : "password"} value={password} onChange={e => setPassword(e.target.value)} placeholder="\u0643\u0644\u0645\u0629 \u0627\u0644\u0645\u0631\u0648\u0631 (8 \u0623\u062D\u0631\u0641 \u0641\u0623\u0643\u062B\u0631)" className="bg-transparent outline-none text-sm text-white placeholder-gray-500 flex-1" dir="ltr" /><button onClick={() => setShowPass(v => !v)} type="button" aria-label={showPass ? "إخفاء كلمة المرور" : "إظهار كلمة المرور"}>{showPass ? /* @__PURE__ */<EyeOff className="w-4 h-4 text-gray-500" /> : /* @__PURE__ */<Eye className="w-4 h-4 text-gray-500" />}</button></div>}{mode === "login" && /* @__PURE__ */<button type="button" onClick={() => {
              setMode("forgot");
              setError("");
