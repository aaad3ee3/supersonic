...
function Footer({
  ...
  return /* @__PURE__ */<footer className="mt-10 mx-4 mb-6 border-t border-purple-950 pt-6"><div className="flex flex-wrap gap-x-5 gap-y-2 text-xs text-purple-400 mb-5">{links.map(([label, msg]) => /* @__PURE__ */<button key={label} onClick={() => showToast(msg)}>{label}</button>)}</div><div className="text-xs text-gray-500 space-y-1 mb-5"><p>خدمة العملاء: support@supersonic.ly</p><p>واتساب: 0910000000</p></div><div className="flex gap-2 mb-5"><button onClick={() => showToast("\u0642\u0631\u064A\u0628\u064B\u0627 \u0639\u0644\u0649 Google Play")} className="flex-1 flex items-center justify-center gap-2 bg-gray-950 border border-purple-900 rounded-xl py-2.5 text-xs text-gray-300"><Download className="w-3.5 h-3.5" /> Google Play</button><button onClick={() => showToast("\u0642\u0631\u064A\u0628\u064B\u0627 \u0639\u0644\u0649 App Store")} className="flex-1 flex items-center justify-center gap-2 bg-gray-950 border border-purple-900 rounded-xl py-2.5 text-xs text-gray-300"><Smartphone className="w-3.5 h-3.5" /> App Store</button></div><p className="text-[10px] text-gray-600 text-center">© Supersonic — جميع الحقوق محفوظة</p></footer>;
}
function RashqSection({
  ...
  return /* @__PURE__ */<div><div className="flex items-center justify-between px-4 mt-7 mb-3"><div className="flex items-center gap-2"><Zap className="w-4 h-4 text-purple-400" /><h3 className="text-white font-bold text-[15px]">رشق — متابعين وتفاعل</h3></div>{apiState === "error" && /* @__PURE__ */<span className="text-[10px] text-amber-500">أسعار تقديرية — الباك اند غير متصل</span>}{apiState === "loading" && /* @__PURE__ */<span className="text-[10px] text-gray-500">يحدّث الأسعار...</span>}</div><div className="px-4 space-y-5">{RASHQ_PLATFORMS.map(p => /* @__PURE__ */<div key={p.key}><div className="flex items-center gap-1.5 mb-2"><p.Icon className="w-3.5 h-3.5 text-purple-400" /><p className="text-purple-300 text-xs font-bold">{p.label}</p></div><div className="space-y-2">{p.services.map(s => {
            ...
            return /* @__PURE__ */<button key={s.id} onClick={() => onOpenService(s, p.label, unitPrice, isLive)} className="w-full flex items-center justify-between bg-gray-950 border border-purple-900 rounded-xl px-3.5 py-3"><ChevronLeft className="w-4 h-4 text-gray-600 shrink-0" /><span className="flex-1 text-right px-3"><span className="block text-gray-200 text-xs">{s.name}</span><span className="block text-purple-500 text-[10px] mt-1">من {fmt(unitPrice)}$ / 1000</span></span></button>;
          })}</div></div>)}</div></div>;
}
function RashqOrderSheet({
  ...
  return /* @__PURE__ */<div className="fixed inset-0 z-30 flex items-end justify-center"><div className="absolute inset-0 bg-black" style={{
      opacity: 0.7
    }} onClick={onClose} /><div className="relative w-full bg-gray-950 border-t border-purple-800 rounded-t-3xl p-5 pb-8" style={{
      maxWidth: 480
    }}><div className="w-10 h-1 bg-purple-800 rounded-full mx-auto mb-5" /><div className="flex items-center justify-between mb-4"><button onClick={onClose} aria-label="إغلاق" className="w-8 h-8 rounded-full bg-purple-950 border border-purple-800 flex items-center justify-center"><X className="w-4 h-4 text-gray-400" /></button><h3 className="text-white font-bold text-xs">{platformLabel} — {service.name}</h3><span className="w-8" /></div><div className="space-y-3 mb-4"><div><label className="text-xs text-gray-400 block mb-1.5">الرابط</label><input value={link} onChange={e => setLink(e.target.value)} placeholder="https://..." className="w-full bg-black border border-purple-900 rounded-xl px-4 py-3 text-white text-sm outline-none" dir="ltr" /></div><div><label className="text-xs text-gray-400 block mb-1.5">الكمية — الحد الأدنى {service.min.toLocaleString("en-US")} والأقصى {service.max.toLocaleString("en-US")}</label><input type="number" value={qty} min={service.min} max={service.max} onChange={e => setQty(Number(e.target.value) || 0)} className="w-full bg-black border border-purple-900 rounded-xl px-4 py-3 text-white text-sm outline-none" /></div></div><div className="flex items-center justify-between bg-purple-950 border border-purple-800 rounded-xl px-4 py-3 mb-2"><span className="text-purple-300 text-xs">السعر الإجمالي {isLive ? "(\u0633\u0639\u0631 \u062D\u064A)" : "(\u062A\u0642\u062F\u064A\u0631\u064A)"}</span><span className="text-white font-bold">${fmt(total)}</span></div>{!backendReady && /* @__PURE__ */<p className="text-amber-500 text-[10px] text-center mb-3">الباك اند مو مربوط بعد — الطلب لن ينفذ فعليًا حتى تضيف رابط Railway بثابت RASHQ_API_BASE</p>}<button onClick={submit} disabled={submitting} className="w-full bg-purple-600 text-white font-bold rounded-xl py-3.5 text-sm flex items-center justify-center gap-2 disabled:opacity-60" style={{
...
function ProductSheet({
  ...
  return /* @__PURE__ */<div className="fixed inset-0 z-30 flex items-end justify-center"><div className="absolute inset-0 bg-black" style={{
      opacity: 0.75
    }} onClick={onClose} /><div className="relative w-full bg-gray-950 border-t border-purple-800 rounded-t-3xl p-5 pb-8 overflow-hidden" style={{
      maxWidth: 480,
      maxHeight: "85vh",
      overflowY: "auto"
            ...
            return /* @__PURE__ */<button key={i} onClick={() => setSelected(p)} className={active ? "rounded-2xl p-3.5 text-right border bg-purple-600 border-purple-400 flex items-center gap-3" : "rounded-2xl p-3.5 text-right border bg-black border-purple-900 flex items-center gap-3"}>{p.image && /* @__PURE__ */<img src={p.image} alt="" loading="lazy" className="w-10 h-10 rounded-lg object-cover shrink-0 bg-gray-900" />}<span className="flex-1 min-w-0"><span className="block text-white text-sm font-bold mb-1 truncate">{p.label}</span><span className={active ? "block text-purple-100 text-xs" : "block text-purple-500 text-xs"}>${fmt(unitPriceOf(p))}</span></span></button>;
...
function SupportView({
  ...
  return /* @__PURE__ */<div className="px-4 pt-4"><div className="rounded-3xl bg-gradient-to-br from-purple-900 to-black border border-purple-800 p-5 text-center mb-6"><h2 className="text-white font-bold text-lg mb-1">التواصل معنا</h2><p className="text-purple-300 text-xs">يا هلا! لا تتردد بالتواصل معنا في حال كان لديك أي استفسار</p></div><div className="space-y-3 mb-8">{items.map((it, i) => {
        ...
        return /* @__PURE__ */<button key={i} onClick={() => showToast(it.msg)} className="w-full flex items-center justify-between bg-gray-950 border border-purple-900 rounded-2xl px-4 py-4"><ChevronLeft className="w-4 h-4 text-gray-600" /><span className="text-sm text-gray-200 flex-1 text-center">{it.label}</span><span className="w-9 h-9 rounded-full bg-purple-700 flex items-center justify-center"><ItIcon className="w-4 h-4 text-white" /></span></button>;
...
function AccountView({
  ...
  return /* @__PURE__ */<div className="px-4 pt-6"><div className="flex flex-col items-center mb-6"><div className="w-20 h-20 rounded-full bg-gradient-to-br from-purple-600 to-purple-950 flex items-center justify-center text-2xl font-black text-white ring-4 ring-purple-700 mb-3" style={{
    ...
    }} className="w-full flex items-center justify-center gap-2 bg-gray-950 border border-purple-900 rounded-xl py-3 text-xs text-gray-300 mb-6"><Copy className="w-3.5 h-3.5" /> نسخ المعرف</button><div className="space-y-2 mb-6">{menu.map((m, i) => {
        const MIcon = m.Icon;
        return /* @__PURE__ */<button key={i} onClick={() => showToast(m.msg)} className="w-full flex items-center justify-between bg-gray-950 border border-purple-900 rounded-2xl px-4 py-3.5"><ChevronLeft className="w-4 h-4 text-gray-600" /><span className="flex items-center gap-2 flex-1 justify-end text-sm text-gray-200">{m.badge && /* @__PURE__ */<span className="text-[10px] bg-purple-600 text-white px-1.5 py-0.5 rounded-full">{m.badge}</span>}{m.label}</span><span className="w-8 h-8 rounded-lg bg-purple-950 flex items-center justify-center ml-2"><MIcon className="w-4 h-4 text-purple-400" /></span></button>;
...
function TopUpSheet({
  ...
  return /* @__PURE__ */<div className="fixed inset-0 z-30 flex items-end justify-center"><div className="absolute inset-0 bg-black" style={{
      ...
      maxWidth: 480
    }}><div className="w-10 h-1 bg-purple-800 rounded-full mx-auto mb-5" /><div className="flex items-center justify-between mb-5"><button onClick={onClose} aria-label="إغلاق" className="w-8 h-8 rounded-full bg-purple-950 border border-purple-800 flex items-center justify-center"><X className="w-4 h-4 text-gray-400" /></button><h3 className="text-white font-bold">شحن الرصيد</h3><span className="w-8" /></div>{step === "form" && /* @__PURE__ */<React.Fragment><div className="flex gap-2 mb-5">{methods.map(m => /* @__PURE__ */<button key={m.id} onClick={() => setMethod(m.id)} className={method === m.id ? "flex-1 py-2 rounded-xl text-[11px] font-bold border bg-purple-600 border-purple-500 text-white" : "flex-1 py-2 rounded-xl text-[11px] font-bold border bg-transparent border-purple-900 text-gray-400"}>{m.label}</button>)}</div>{method === "card" && /* @__PURE__ */<div className="space-y-3"><label className="text-xs text-gray-400">أدخل كود كرت الشحن</label><input value={code} onChange={e => setCode(e.target.value)} placeholder="XXXX-XXXX-XXXX" className="w-full bg-black border border-purple-900 rounded-xl px-4 py-3 text-white text-sm outline-none" /></div>}{method === "bank" && /* @__PURE__ */<div className="space-y-3"><input placeholder="\u0631\u0642\u0645 \u0627\u0644\u0628\u0637\u0627\u0642\u0629" className="w-full bg-black border border-purple-900 rounded-xl px-4 py-3 text-white text-sm outline-none" /><div className="flex gap-3"><input placeholder="MM/YY" className="w-1/2 bg-black border border-purple-900 rounded-xl px-4 py-3 text-white text-sm outline-none" /><input placeholder="CVV" className="w-1/2 bg-black border border-purple-900 rounded-xl px-4 py-3 text-white text-sm outline-none" /></div></div>}{method === "transfer" && /* @__PURE__ */<div className="space-y-3"><div className="flex items-center gap-3"><select className="flex-1 bg-black border border-purple-900 rounded-xl px-3 py-3 text-white text-sm outline-none"><option>من: دينار ليبي</option><option>من: دولار</option></select><ArrowLeftRight className="w-4 h-4 text-purple-500" /><select className="flex-1 bg-black border border-purple-900 rounded-xl px-3 py-3 text-white text-sm outline-none"><option>إلى: دولار</option><option>إلى: مصرف</option></select></div><input placeholder="\u0627\u0644\u0645\u0628\u0644\u063A" className="w-full bg-black border border-purple-900 rounded-xl px-4 py-3 text-white text-sm outline-none" /></div>}<button onClick={submit} className="w-full bg-purple-600 text-white font-bold rounded-xl py-3.5 text-sm mt-5" style={{
          boxShadow: "0 0 24px rgba(168,85,247,0.45)"
        }}>تأكيد</button></React.Fragment>}{step === "otp" && /* @__PURE__ */<div><p className="text-gray-400 text-xs text-center mb-4">أدخل كود التحقق المرسل إلى رقم هاتفك</p><div className="flex justify-center gap-2 mb-5" dir="ltr">{[0, 1, 2, 3].map(i => /* @__PURE__ */<input key={i} maxLength={1} className="w-11 h-12 text-center bg-black border border-purple-800 rounded-xl text-white text-lg outline-none" />)}</div><button onClick={confirmOtp} className="w-full bg-purple-600 text-white font-bold rounded-xl py-3.5 text-sm" style={{
          boxShadow: "0 0 24px rgba(168,85,247,0.45)"
...
function ResetPasswordScreen({
  ...
  return /* @__PURE__ */<div className="min-h-screen bg-black flex justify-center"><div className="w-full flex flex-col justify-center px-6 relative overflow-hidden" style={{
      maxWidth: 480,
      minHeight: "100vh"
        ...
        width: "100%"
            ...
            boxShadow: "0 0 24px rgba(168,85,247,0.45)"
          }}>تسجيل الدخول</button></div> : /* @__PURE__ */<form onSubmit={submit}><h2 className="text-white font-bold text-lg mb-1 text-center">كلمة مرور جديدة</h2><p className="text-gray-500 text-xs mb-6 text-center">اكتب كلمة مرور جديدة لحسابك</p><div className="flex items-center gap-2 bg-gray-950 border border-purple-900 rounded-xl px-4 py-3 mb-3"><Lock className="w-4 h-4 text-purple-500" /><input type={showPass ? "text" : "password"} value={password} onChange={e => setPassword(e.target.value)} placeholder="\u0643\u0644\u0645\u0629 \u0627\u0644\u0645\u0631\u0648\u0631 \u0627\u0644\u062C\u062F\u064A\u062F\u0629 (8 \u0623\u062D\u0631\u0641 \u0641\u0623\u0643\u062B\u0631)" className="bg-transparent outline-none text-sm text-white placeholder-gray-500 flex-1" dir="ltr" /><button onClick={() => setShowPass(v => !v)} type="button" aria-label={showPass ? "إخفاء كلمة المرور" : "إظهار كلمة المرور"}>{showPass ? /* @__PURE__ */<EyeOff className="w-4 h-4 text-gray-500" /> : /* @__PURE__ */<Eye className="w-4 h-4 text-gray-500" />}</button></div>{error && /* @__PURE__ */<p className="text-red-400 text-xs text-center mb-3">{error}</p>}<button type="submit" disabled={loading} className="w-full bg-purple-600 text-white font-bold rounded-xl py-3.5 text-sm disabled:opacity-60 flex items-center justify-center gap-2" style={{
            boxShadow: "0 0 24px rgba(168,85,247,0.45)"
...
function AuthScreen({
  ...
  return /* @__PURE__ */<div className="min-h-screen bg-black flex justify-center"><div className="w-full flex flex-col md:flex-row" style={{
      maxWidth: 1100
        ...
        minHeight: "100vh"
      }}><div className="md:hidden"><GlowOrbs /><SpeedStreaks /></div><div className="relative z-10 w-full" style={{
          maxWidth: 380,
          margin: "0 auto"
              ...
              setMode("login");
              setForgotSent(false);
            }} className="text-purple-400 text-xs font-bold">رجوع لتسجيل الدخول</button></div> : /* @__PURE__ */<form onSubmit={submit} className="space-y-3">{mode === "signup" && /* @__PURE__ */<div className="flex items-center gap-2 bg-gray-950 border border-purple-900 rounded-xl px-4 py-3"><User className="w-4 h-4 text-purple-500" /><input value={name} onChange={e => setName(e.target.value)} placeholder="\u0627\u0633\u0645\u0643" className="bg-transparent outline-none text-sm text-white placeholder-gray-500 flex-1" /></div>}<div className="flex items-center gap-2 bg-gray-950 border border-purple-900 rounded-xl px-4 py-3"><Mail className="w-4 h-4 text-purple-500" /><input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="\u0627\u0644\u0628\u0631\u064A\u062F \u0627\u0644\u0625\u0644\u0643\u062A\u0631\u0648\u0646\u064A" className="bg-transparent outline-none text-sm text-white placeholder-gray-500 flex-1" dir="ltr" /></div>{mode !== "forgot" && /* @__PURE__ */<div className="flex items-center gap-2 bg-gray-950 border border-purple-900 rounded-xl px-4 py-3"><Lock className="w-4 h-4 text-purple-500" /><input type={showPass ? "text" : "password"} value={password} onChange={e => setPassword(e.target.value)} placeholder="\u0643\u0644\u0645\u0629 \u0627\u0644\u0645\u0631\u0648\u0631 (8 \u0623\u062D\u0631\u0641 \u0641\u0623\u0643\u062B\u0631)" className="bg-transparent outline-none text-sm text-white placeholder-gray-500 flex-1" dir="ltr" /><button onClick={() => setShowPass(v => !v)} type="button" aria-label={showPass ? "إخفاء كلمة المرور" : "إظهار كلمة المرور"}>{showPass ? /* @__PURE__ */<EyeOff className="w-4 h-4 text-gray-500" /> : /* @__PURE__ */<Eye className="w-4 h-4 text-gray-500" />}</button></div>}{mode === "login" && /* @__PURE__ */<button type="button" onClick={() => {
              setMode("forgot");
              setError("");
...
function App() {
  ...
  if (authChecking) {
    return /* @__PURE__ */<div className="min-h-screen bg-black flex items-center justify-center"><Loader2 className="w-7 h-7 text-purple-500 animate-spin" /></div>;
  }
  if (!currentUser) {
  ...
  return /* @__PURE__ */<div dir="rtl" className="min-h-screen bg-black flex justify-center"><div className="w-full relative text-white font-sans" style={{
      maxWidth: 480
